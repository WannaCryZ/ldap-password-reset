<?php
include_once $_SERVER['DOCUMENT_ROOT'] . '/extend/PHPMailer.php';
...
function sendMail($to, $recipinetname, $username, $code, &$result)
{
    ...
    try {
        ...
        //生成重置密码的链接
        $reset_password_link = RESET_LINK . 'reset_check.php?username=' . $username . '&code=' . $code; // 替换为你的重置密码页面链接
        $mail->Subject = '【IT支持】LDAP账号重置密码'; // 设置邮件主题

        $mail_content = '您好，' . $recipinetname . '：<br><br>您申请了LDAP账号密码重置，<br><br>请点击链接重置密码：' . '<a href="' . $reset_password_link . '">[重置密码链接]</a>（链接有效期24小时）' .
            '<br><br>如果您无法点击链接，请将以下地址复制到浏览器地址栏中：<br><br>' . $reset_password_link .
            '<br><br>如果您没有申请重置密码，请忽略此邮件。<br><br>
        如果您在登录或使用过程中遇到任何问题，请随时联系IT支持（企微服务号）。<br><br>
			获取更多信息：<br><br>- IT官网：<a href="https://it.example.com/">https://it.example.com/</a>（需公司内网访问）<br><br>
            <br>'; // 设置邮件正文
        $mail_sign = '<div class="qqmail_sign" id="wemailsigcontent" signid="200">
            <div class="rich_custom_signature" style="position: relative;">
                <p><b>IT组 </b><b>| </b><b>公司</b><b>-运维与安全部</b></p>
...
function sendMailNotice($to, $recipinetname, $username, &$result)
{
    ...
    try {
        ...
        $mail->Subject = '【IT支持】LDAP账号密码修改通知'; // 设置邮件主题
        $mail_content = '您好，' . $recipinetname . '：<br><br>您已成功修改了LDAP账号（' . $username . '）的密码，请牢记并保管好您的新密码。' .
            '<br><br>如果不是您本人操作的修改，请立即联系IT支持（企微服务号）。<br><br><br><br>
			获取更多信息：<br><br>- IT官网：<a href="https://it.example.com/">https://it.example.com/</a>（需公司内网访问）<br><br>
            <br>'; // 设置邮件正文
        $mail_sign = '<div class="qqmail_sign" id="wemailsigcontent" signid="200">
            <div class="rich_custom_signature" style="position: relative;">
                <p><b>IT组 </b><b>| </b><b>公司</b><b>-</b><b>IT数字化部</b></p>
                <div>重要提示：本邮件及附件具保密性质，可能包含商业秘密及根据法律享有特权或不得披露的信息。</div>
                <div>如果您意外收到此邮件，特此提醒您本邮件的机密性，请立即回复邮件通知我们并从您的系统中删除本邮件及附件。</div>
                <div>如果您不是本邮件应当的收件人，请注意不可利用、复制本邮件及其附件内容或向他人披露该等内容。</div>
            </div>
